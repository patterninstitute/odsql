...
validate_select <- function(value) {
  ...
  if (length(bad)) {
    cli::cli_abort(c(
      "x" = "Invalid field name(s) in select clause: {bad}"
    ))
  }
...
validate_limit <- function(value) {
  if (!is.numeric(value) || length(value) != 1 || is.na(value)) {
    cli::cli_abort(c(
      "x" = "The limit argument must be a non-missing scalar number."
    ))
  }
  ...
  if (value < -1 || value > 100) {
    cli::cli_abort(c(
      "x" = "The limit argument must be between -1 and 100."
    ))
  }
...
validate_group_by <- function(value) {
  ...
  # Use the following validations from select clause:
  # - check extra commas
